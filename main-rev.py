...
def build_train_vectors(training_file, language):
	'''
	Parses the *-train.xml file, apply feature extraction
	returns dict:
		lexelt: {list_of_all_words_of_lexelt, list_of_all_senseids, list_of_all_context_vectors}
	'''
	data = {} 
...
def build_dev_data(dev_file, language):
	'''
	Parses the *-dev.xml file, apply feature extraction
	returns dict for disambiguation:
		lexelt: {list_of_all_2-tuples_(instanceid, context)}
	'''
	data = {} 
...
def build_dict(train_data):
	...
	for lexelt in train_data:
		...
		model_dict[lexelt] = svm_clf
	return model_dict

def disambiguate(language, model, train_data, dev_data, answer_file):
	outfile_svm = codecs.open(answer_file, encoding = 'utf-8', mode = 'w')
	# outfile_svm = codecs.open(language + '.answer', encoding = 'utf-8', mode = 'w')
	...
	for lexelt, id_ctxt_tuples in sorted(dev_data.iteritems(), key = lambda d: replace_accented(d[0].split('.')[0])):
		for each_id_ctxt_tuple in sorted(id_ctxt_tuples, key = lambda d: str(d[0].split('.')[-1])):
			...
			# Predict
			svm_predict_sense_id = model[lexelt].predict(context_vector)[0]
...
if __name__ == '__main__':
	...
	language = sys.argv[4]
	train_data = build_train_vectors(training_file, language)
	model = build_dict(train_data)
	dev_data = build_dev_data(dev_file, language)
	disambiguate(language, model, train_data, dev_data, answer_file)
